...
-- *** // *** // *** CODIGO AUXILIAR CON ETYPE *** // *** // ***
data EType = ENat | EBool

implementation DecEq EType where
  decEq ENat ENat = Yes Refl
  decEq EBool EBool = Yes Refl
  
  
interpEType : EType -> Type
interpEType ENat = Nat
interpEType EBool = Bool

data DeleteLabelAtPred_EType: (String, EType) -> List (String, EType) -> List (String, EType) -> Type where
  EmptyRecord_EType : DeleteLabelAtPred_EType l [] []
  IsElem_EType : DeleteLabelAtPred_EType l (l :: ls) ls
  IsNotElem_EType : Not (l1 = l2) -> DeleteLabelAtPred_EType l1 ls1 ls2 -> 
    DeleteLabelAtPred_EType l1 (l2 :: ls1) (l2 :: ls2)

data DeleteLabelsPred_EType : List (String, EType) -> List (String, EType) -> List (String, EType) -> Type where
  EmptyLabelList_EType : DeleteLabelsPred_EType [] ls ls
  DeleteFirstOfLabelList_EType : DeleteLabelAtPred_EType l lsAux lsRes -> DeleteLabelsPred_EType ls1 ls2 lsAux ->
    DeleteLabelsPred_EType (l :: ls1) ls2 lsRes

data IsLeftUnion_EType : List (String, EType) -> List (String, EType) -> List (String, EType) -> Type where
  IsLeftUnionAppend_EType : DeleteLabelsPred_EType ls1 ls2 ls3 -> IsLeftUnion_EType ls1 ls2 (ls1 ++ ls3)

fromDeleteLabelAtListFuncToPred2 : DeleteLabelAtPred_EType l ls (deleteAtList l ls)
fromDeleteLabelAtListFuncToPred2 {l} {ls = []} = EmptyRecord_EType
fromDeleteLabelAtListFuncToPred2 {l=l1} {ls = (l2 :: ls)} = ?fromDeleteLabel2_rhs

fromDeleteLabelsListFuncToPred2 : DeleteLabelsPred_EType ls1 ls2 (deleteList ls1 ls2)
fromDeleteLabelsListFuncToPred2 {ls1 = []} {ls2} = EmptyLabelList_EType
fromDeleteLabelsListFuncToPred2 {ls1 = (l :: ls1)} {ls2} =
  let subDelListPred = fromDeleteLabelsListFuncToPred2 {ls1} {ls2}
      delAtPred = fromDeleteLabelAtListFuncToPred2 {l} {ls=deleteList ls1 ls2}
  in DeleteFirstOfLabelList_EType {lsAux=deleteList ls1 ls2} delAtPred subDelListPred

fromLeftUnionFuncToPred2 : IsLeftUnion_EType ls1 ls2 (leftUnion ls1 ls2)   
fromLeftUnionFuncToPred2 {ls1} {ls2} =
  let delPred = fromDeleteLabelsListFuncToPred2 {ls1} {ls2}
  in IsLeftUnionAppend_EType delPred

data VarDec2 : String -> EType -> Type where
  (:-:) : (var : String) -> (ety : EType) -> VarDec2 var ety

infixr 2 :-:

data ValDec2 : String -> EType -> Type where
  MkValDec2 : (var : String) -> (ety : EType) -> interpEType ety -> ValDec2 var ety

syntax [var] as [ety] is [val] = MkValDec2 var ety val;

data Exp2 : List (String, EType) -> EType -> Type where
  Add2 : Exp2 fvs1 ENat -> Exp2 fvs2 ENat -> IsLeftUnion_EType fvs1 fvs2 fvsRes-> Exp2 fvsRes ENat 
  If2 : Exp2 fvs EBool -> Exp2 fvsT ety -> Exp2 fvsF ety -> SuperLeftUnion fvs fvsT fvsF fvsRes -> Exp2 fvsRes ety
  Var2 : VarDec2 var ety -> Exp2 [(var, ety)] ety
  Cons2 : interpEType ety -> Exp2 [] ety
  Let2 : ValDec2 var ety -> Exp2 fvsInner ety -> DeleteLabelAtPred_EType (var, ety) fvsInner fvsOuter -> Exp2 fvsOuter ety

data IsSubSet2 : List (String, EType) -> List (String, EType) -> Type where
  IsSubSetNil2 : IsSubSet2 [] ls
  IsSubSetCons2 : IsSubSet2 ls1 ls2 -> Elem l ls2 -> IsSubSet2 (l :: ls1) ls2
  
-- DSL del lenguaje
var2 : VarDec2 var ety -> Exp2 [(var, ety)] ety
var2 varDec = Var2 varDec

cons2 : interpEType ety -> Exp2 [] ety
cons2 val = Cons2 val

cons3 : (ety : EType) -> interpEType ety -> Exp2 [] ety
cons3 _ val = Cons2 val

add2 : Exp2 fvs1 ety -> Exp2 fvs2 ety -> Exp2 (leftUnion fvs1 fvs2) ety
add2 {fvs1} {fvs2} e1 e2 = Add2 e1 e2 (fromLeftUnionFuncToPred2 {ls1=fvs1} {ls2=fvs2})

eLet2 : ValDec2 var ety -> Exp2 fvs ety -> Exp2 (deleteAtList (var, ety) fvs) ety
eLet2 {var} {ety} {fvs} valDec e = Let2 valDec e (fromDeleteLabelAtListFuncToPred2 {l=(var, ety)} {ls=fvs})  
  
-- Ejemplos --
eTypeTest1 : Exp2 [("x", ENat), ("y", ENat)] ENat
eTypeTest1 = add2 (var2 ("x" :-: ENat)) (add2 (var2 ("y" :-: ENat)) (cons2 10))

eTypeTest2 : Exp2 [("y", ENat)] ENat
eTypeTest2 = eLet2 ("x" as ENat is 10) eTypeTest1

eTypeTest3 : Exp2 [] ENat
eTypeTest3 = eLet2 ("y" as ENat is 3) eTypeTest2
  
-- Interprete  
  
interpETypePair : (String, EType) -> (String, Type)
interpETypePair (v, ety) = (v, interpEType ety)  
  
data Ambiente2 : List (String, EType) -> Type where
  MkAmbiente2 : Record {lty=String} (map (\p => interpETypePair p) fvs) -> Ambiente2 fvs

ifHasFieldInElemThenItHasThere2 : Elem (l, ety) ls -> HasField l (map (\p => interpETypePair p) ls) (interpEType ety)

ifIsSubSetThenHasFieldInIt2 : {ety : EType} -> IsSubSet2 ls1 ls2 -> 
  HasField l (map (\p => interpETypePair p) ls1) (interpEType ety) -> HasField l (map (\p => interpETypePair p) ls2) (interpEType ety)
ifIsSubSetThenHasFieldInIt2 {ls1 = []} _ hasField = absurd $ noEmptyHasField hasField
ifIsSubSetThenHasFieldInIt2 {ety} {l=l1} {ls1 = (l2 :: ls1)} (IsSubSetCons2 isElem subSet) hasField with (decEq (l1, ety) l2)
  ifIsSubSetThenHasFieldInIt2 {ety} {l=l1} {ls1 = ((l1, ety) :: ls1)} (IsSubSetCons2 subSet isElem) hasField | Yes Refl = ifHasFieldInElemThenItHasThere2 isElem
  ifIsSubSetThenHasFieldInIt2 {ety} {l=l1} {ls1 = (l2 :: ls1)} (IsSubSetCons2 isElem subSet) hasField | No notL1EqL2 = ?subSetField_rhs_2
  
interpEnv2 : Ambiente2 fvsEnv -> IsSubSet2 fvs fvsEnv -> Exp2 fvs ety -> interpEType ety
interpEnv2 env subSet (Add2 e1 e2 leftUnion) = ?interpEnv2_rhs_1
interpEnv2 (MkAmbiente2 recEnv) subSet (Var2 (v :-: ety)) = 
  -- Al ser un subconjunto del ambiente, la variable existe en el y se puede obtener
  let hasField = HasFieldHere {l=v} {ty = interpEType ety} {ts = []}
      hasFieldInEnv = ifIsSubSetThenHasFieldInIt2 {ety} subSet hasField
  in hLookupByLabel v recEnv hasFieldInEnv
interpEnv2 env subSet (Cons2 val) = val
interpEnv2 env subSet (Let2 valDec e delAt) = ?interpEnv2_rhs_4

interp2 : Exp2 [] ty -> interpEType ty
interp2 = interpEnv2 (MkAmbiente2 {fvs=[]} emptyRec) IsSubSetNil2
